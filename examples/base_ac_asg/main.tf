...
# via a hardcoded `data "zpa_enrollment_cert"` (name = "Connector").
################################################################################
...
#
# Every VM the autoscaler launches enrolls with the SAME provisioning key
# (baked into the instance template's user_data below). Bumping the key max
# usage or creating a fresh key requires re-applying the template.
################################################################################
...
#    Connector registration. Used if variable use_zscaler_image is set to true.
################################################################################
locals {
...
################################################################################
# B. RHEL9 user_data variant. Used if variable use_zscaler_image is set to false.
################################################################################
locals {
...
################################################################################
# Locate Latest App Connector Image on Google Marketplace by Project and Name
################################################################################
...
################################################################################
# 5. Create the App Connector autoscaling group(s).
#    One MIG + one Autoscaler per zone in zones_list.
#    Group sizes (min_size / max_size) are PER ZONE.
################################################################################
module "ac_asg" {
