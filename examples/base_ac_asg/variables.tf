...
variable "az_count" {
  type        = number
  description = "Default number of zonal MIGs to create based on availability zone count. One MIG + one autoscaler is created per zone."
  default     = 1
  validation {
    condition = (
...
################################################################################
# Autoscaling controls passed through to terraform-zsac-asg-gcp
################################################################################
...
variable "target_tracking_metric" {
  type        = string
  description = "Target tracking metric for the autoscaling policy. Names mirror AWS predefined metric types so callers can switch clouds without renaming. Approved values: ASGAverageCPUUtilization, ASGAverageNetworkIn, ASGAverageNetworkOut."
...
variable "target_cpu_util_value" {
  type        = number
  description = "Target value for the autoscaling policy. For ASGAverageCPUUtilization this is interpreted as a CPU utilization percentage (1-100). For ASGAverageNetworkIn/Out this is interpreted as bytes-per-second per instance."
  default     = 50
}

variable "cooldown_period" {
  type        = number
  description = "Number of seconds the autoscaler waits before it starts collecting information from a new instance."
  default     = 60
}

variable "health_check_grace_period" {
  type        = number
  description = "Time in seconds the MIG autohealing health check waits before checking a newly created instance."
  default     = 300
}
...
variable "provisioning_key_max_usage" {
  type        = number
  description = "The maximum number of instances where this provisioning key can be used for enrolling an App Connector or Service Edge. For autoscaling, set this comfortably above var.max_size * length(var.zones) to allow for instance churn."
  default     = 100
}
